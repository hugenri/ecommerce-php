...
use App\Modules\Deliveries\Application\RegisterShippingDateUseCase as DeliveriesRegisterShippingDateUseCase;
use App\Modules\EmployeeDashboard\Domain\EmployeeDashboardRepositoryInterface;

class RegisterShippingDateUseCase
{
    public function __construct(
        private EmployeeDashboardRepositoryInterface $repository,
        private DeliveriesRegisterShippingDateUseCase $deliveriesRegisterShippingDate,
    ) {}

    public function execute(int $deliveryId, string $date, int $userId): void
    {
        $this->assertCanModify($deliveryId, $userId);
        $this->deliveriesRegisterShippingDate->execute($deliveryId, $date);
    }
